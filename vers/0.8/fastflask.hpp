...
void get_response(
    ...
    DWORD read;
    ReadFile(pipe, buf, 1023 * sizeof(char), &read, NULL);
    buf[read] = '\0';
...
void start(){
    ...
    // handled pipes set
    std::unordered_set<std::string> handled_pipes;
    ...
    while (true){
        ...
        if (hFind == INVALID_HANDLE_VALUE){
            ...
            do {
                ...
                if (!string_starts_with(pipe_name, "\\\\.\\pipe\\.requests") || (handled_pipes.find(pipe_name) != handled_pipes.end())){
                    continue;
                }
                handled_pipes.insert(pipe_name);

                // spawn thread to return resposne
                std::thread resp_thread(get_response, w_pipe_name, pipe_name);
                resp_thread.detach();
