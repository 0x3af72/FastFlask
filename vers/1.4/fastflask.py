...
def start(exe_path, host="127.0.0.1", port=5000):
    ...
    def cpp_backend(e):
        ...
        to_exec, to_return, template, kwargs = api_response
        resp = flask.make_response() if not template else flask.make_response(render_template(template, **(kwargs if isinstance(kwargs, dict) else {})))
        exec(to_exec)
        if not template: resp.set_data(to_return)
        return resp
