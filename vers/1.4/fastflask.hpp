...
// response object
struct RES{
    std::string to_exec, to_return, r_template;
    int code = 200;
    json kwargs;
    RES(std::string to_exec, std::string to_return, std::string r_template = "", json kwargs = json({}), int code = 200):
    to_exec(to_exec), to_return(to_return), r_template(r_template), kwargs(kwargs), code(code){}
};
...
void get_response(
    std::unordered_map<std::string, int>& done_pipes,
    ...
    std::string r_template; json kwargs = json({});
    if (route_exists){
    ...
    // return response
    json json_resp = {to_exec, to_return, r_template, kwargs};
    ...
    done_pipes[pipe_name] = REMOVE_TICKS;
}
...
void start(){
    ...
    // currently being handled and to remove
    ...
    while (true){
        ...
        if (hFind == INVALID_HANDLE_VALUE){
            ...
            do {
                ...
                std::thread resp_thread(get_response, std::ref(done_pipes), w_pipe_name, pipe_name);
                resp_thread.detach();
        ...
        // remove done pipes
        std::vector<std::string> erased_pipes;
        for (auto& [done_pipe, t]: done_pipes){
            if (t == 0){
                handling_pipes.erase(handling_pipes.find(done_pipe));
                erased_pipes.push_back(done_pipe);
            } else {
                t -= 1;
            }
        }
        for (std::string erased: erased_pipes) done_pipes.erase(done_pipes.find(erased));
    }
}
